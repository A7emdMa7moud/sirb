...
function MovieSection({ sectionTitle, requestKey }) {
  ...
  const fetchMovies = async () => {
    try {
      ...
      const requestUrl = movieRequests[requestKey];
      if (!requestUrl) {
      ...
      console.error(`Error fetching ${sectionTitle} movies:`, error);
      setErrorMessage(error.message || `فشل في جلب أفلام ${sectionTitle}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <section
        className={`trending movie-section flex flex-col gap-4 my-10 mb-20 transition-opacity duration-500 ${
      ...
      >
        ...
        <div className="relative">
          ...
          >
            {movieList.slice(0, 20).map((movie, index) => (
              <motion.li
                key={movie.id}
                variants={{
