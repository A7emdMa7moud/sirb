...
function MovieCard({ movie, index }) {
  ...
  const handleShare = async (e) => {
    ...
    const url = `https://sirb-two.vercel.app/movies/${movie.id}`;
    const title = `${movie.title} | منصة سيرب`;
    const text = `شاهد فيلم ${
      movie.title
    } على منصة سيرب - ${movie.overview?.slice(0, 100)}...`;
    const imageUrl = movie.poster_path
      ? `https://image.tmdb.org/t/p/w500${movie.poster_path}`
      ...
      <div className="mt-2 w-full flex flex-col max-w-[180px]">
        ...
        <div className="flex justify-between items-center gap-4 mt-1">
          ...
          <button
            ...
            aria-label={`مشاركة فيلم ${movie.title}`}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
